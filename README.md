...
Подключитесь к нему по SSH от имени пользователя `root` и выполните **ровно одну команду** (для защиты от обрывов связи мы задаем новый порт SSH напрямую в команде):

```bash
SSH_PORT=2222 wget -qO- https://raw.githubusercontent.com/blablajka/serv/main/deploy.sh | bash
```

> **Важно:** Скрипт полностью автоматизирован. Он сам перенесет SSH на порт `2222` (вы можете поменять его в команде), настроит Firewall, заблокирует сканеры РКН и автоматически поднимет Zapret. Если у вас на сервере были настроены SSH-ключи, скрипт также отключит вход по паролю.
